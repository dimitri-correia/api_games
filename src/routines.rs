...
pub async fn action_for_char(char: &str, server_clone: Arc<Server>, map: Arc<Map>) {
    let map = &*map;
    loop {
        // wait cooldown if any
        let cooldown = character::get_char_infos(&server_clone, char).await.cooldown as f32;
        ...
        let max_item = character::get_char_infos(&server_clone, char).await.inventory_max_items;
        ...
        // gather resource
        if char.eq("dim") {
            handle_action_with_cooldown(&server_clone, Action::Fight, char, 300, None).await;
            continue;
        }
        handle_action_with_cooldown(&server_clone, Action::Gathering, char, 100, None).await;
    }
}
